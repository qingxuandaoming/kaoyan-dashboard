...
EDITS = [
    {
        ...
        "replacements": [
            ...
            {
                "old": "一键运行：`python E:\\NPEE\\src\\run_pipeline.py`",
                "new": "一键运行：`python E:\\NPEE\\src\\run_pipeline.py`（`--no-plan` 跳过每日计划生成）\n\n打开大盘：双击桌面 `启动考研大盘.bat`（刷新数据 → 启动 serve.js → 打开 http://localhost:8080/dashboard.html）",
            },
            {
